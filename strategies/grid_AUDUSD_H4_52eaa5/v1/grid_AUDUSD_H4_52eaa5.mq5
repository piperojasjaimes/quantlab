//+------------------------------------------------------------------+
//| grid_AUDUSD_H4_52eaa5.mq5
//| Generated by QuantLab — 2026-07-04T02:23:57.976898+00:00
...
#define MAGIC_NUMBER 348474

int emaFastHandle, emaSlowHandle, adxHandle, atrHandle;

int OnInit() {
    emaFastHandle = iMA(_Symbol, PERIOD_CURRENT, 9, 0, MODE_EMA, PRICE_CLOSE);
    emaSlowHandle = iMA(_Symbol, PERIOD_CURRENT, 68, 0, MODE_EMA, PRICE_CLOSE);
    adxHandle = iADX(_Symbol, PERIOD_CURRENT, 14);
    atrHandle = iATR(_Symbol, PERIOD_CURRENT, 17);
    return INIT_SUCCEEDED;
}
...
void ExecuteStrategy() {
    ...
    if(adxVal[1] < 28) return;

    if(emaFast[2] < emaSlow[2] && emaFast[1] > emaSlow[1] && adxVal[1] > 28) {
        double sl = ask - InpStopLoss * _Point;
        double tp = ask + InpTakeProfit * _Point;
        trade.Buy(InpLotSize, _Symbol, ask, sl, tp, "grid_AUDUSD_H4_52eaa5_BUY");
    }
    if(emaFast[2] > emaSlow[2] && emaFast[1] < emaSlow[1] && adxVal[1] > 28) {
        double sl = bid + InpStopLoss * _Point;
        double tp = bid - InpTakeProfit * _Point;
        trade.Sell(InpLotSize, _Symbol, bid, sl, tp, "grid_AUDUSD_H4_52eaa5_SELL");
    }
}
