//+------------------------------------------------------------------+
//| scalping_EURUSD_M5_606dae.mq5
//| Generated by QuantLab — 2026-07-04T02:15:10.208889+00:00
//| Pattern: scalping | TF: M5 | Symbol: EURUSD
//+------------------------------------------------------------------+
...
input int    InpStopLoss = 13;
input int    InpTakeProfit = 51;
...
#define MAGIC_NUMBER 103954

int emaFastHandle, emaSlowHandle;

int OnInit() {
    emaFastHandle = iMA(_Symbol, PERIOD_CURRENT, 6, 0, MODE_EMA, PRICE_CLOSE);
    emaSlowHandle = iMA(_Symbol, PERIOD_CURRENT, 55, 0, MODE_EMA, PRICE_CLOSE);
    return INIT_SUCCEEDED;
}
...
void ExecuteStrategy() {
    ...
    if(emaFast[1] > emaSlow[1] && emaFast[0] > emaSlow[0]) {
        ...
        trade.Buy(InpLotSize, _Symbol, ask, sl, tp, "scalping_EURUSD_M5_606dae_BUY");
    }
    if(emaFast[1] < emaSlow[1] && emaFast[0] < emaSlow[0]) {
        double sl = bid + InpStopLoss * _Point;
        double tp = bid - InpTakeProfit * _Point;
        trade.Sell(InpLotSize, _Symbol, bid, sl, tp, "scalping_EURUSD_M5_606dae_SELL");
    }
}
